<template>
  <div class="min-h-screen pt-28 pb-12 px-4 sm:px-6 lg:px-8 container mx-auto" dir="rtl">
    
    <!-- خوش‌آمدگویی -->
    <div class="mb-8 animate-fade-up">
      <h1 class="text-3xl md:text-4xl font-extrabold text-slate-900 dark:text-white mb-2">
        سلام، <span class="text-transparent bg-clip-text bg-gradient-to-l from-cyan-500 to-purple-600">{{ username }}</span>! 👋
      </h1>
      <p class="text-slate-600 dark:text-slate-400">به پنل کاربری خود خوش آمدید.</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-up" style="animation-delay: 0.1s;">
      ...
      <div class="bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl p-6 md:p-8 rounded-[2.5rem] border border-slate-200/50 dark:border-white/5 shadow-xl relative overflow-hidden group">
        ...
        <div class="flex items-center gap-4 mb-6">
          ...
          <h2 class="text-xl font-bold text-slate-900 dark:text-white">اطلاعات کاربری</h2>
        </div>
        <div class="space-y-3">
          <div class="flex justify-between border-b border-slate-200/50 dark:border-slate-800/50 pb-2">
            <span class="text-slate-500 dark:text-slate-400 text-sm">نام کاربری:</span>
            <span class="font-medium text-slate-800 dark:text-slate-200">{{ username }}</span>
          </div>
          <div class="flex justify-between border-b border-slate-200/50 dark:border-slate-800/50 pb-2">
            <span class="text-slate-500 dark:text-slate-400 text-sm">وضعیت حساب:</span>
            <span class="font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-500/10 px-2 py-0.5 rounded-lg text-xs">فعال</span>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl p-6 md:p-8 rounded-[2.5rem] border border-slate-200/50 dark:border-white/5 shadow-xl relative overflow-hidden group">
        ...
        <div class="flex items-center gap-4 mb-6">
          ...
          <h2 class="text-xl font-bold text-slate-900 dark:text-white">درخواست‌های من</h2>
        </div>
        ...
        <div v-else-if="myRequests.length === 0" class="flex flex-col items-center justify-center py-4 text-center">
          <p class="text-slate-500 dark:text-slate-400 text-sm mb-4">تا کنون درخواست مشاوره‌ای ثبت نکرده‌اید.</p>
          <NuxtLink to="/contact" class="text-sm font-bold text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 transition-colors">ثبت درخواست جدید &larr;</NuxtLink>
        </div>
        ...
        <div v-else class="space-y-3 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
          ...
          <div class="pt-2 text-center border-t border-slate-200/50 dark:border-slate-800/50 mt-2">
            <NuxtLink to="/contact" class="text-xs font-bold text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 transition-colors">ثبت درخواست جدید &larr;</NuxtLink>
          </div>
        </div>
      ...
      <div class="bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl p-6 md:p-8 rounded-[2.5rem] border border-slate-200/50 dark:border-white/5 shadow-xl relative overflow-hidden group">
        ...
        <div class="flex items-center gap-4 mb-6">
          ...
          <h2 class="text-xl font-bold text-slate-900 dark:text-white">تنظیمات</h2>
        </div>
        <div class="space-y-4">
          <button @click="showPasswordModal = true" class="w-full text-right px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors text-sm font-medium">تغییر رمز عبور</button>
          <button @click="logout" class="w-full text-right px-4 py-2 rounded-xl bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-500/20 transition-colors text-sm font-medium border border-transparent dark:border-red-500/10">خروج از حساب</button>
        </div>
      </div>
    ...
    <div v-if="showPasswordModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm transition-opacity">
      <div class="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-2xl p-6 md:p-8 w-full max-w-md relative animate-fade-up">
        ...
        <h3 class="text-xl font-bold text-slate-900 dark:text-white mb-6">تغییر رمز عبور</h3>

        <form @submit.prevent="handleChangePassword" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">رمز عبور فعلی</label>
          ...
          <div>
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">رمز عبور جدید</label>
          ...
          <div class="flex gap-3 pt-4">
            <button type="submit" :disabled="passPending" class="flex-1 bg-gradient-to-r from-cyan-500 to-blue-500 text-white font-bold py-3 px-4 rounded-xl hover:from-cyan-600 hover:to-blue-600 focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center">
              <span v-if="passPending">در حال ثبت...</span>
              <span v-else>ذخیره تغییرات</span>
            </button>
            <button type="button" @click="showPasswordModal = false" class="flex-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold py-3 px-4 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">لغو</button>
          </div>
        </form>
      </div>
    </div>
...
const handleChangePassword = async () => {
  ...
  try {
    ...
    passSuccess.value = response.message || 'رمز عبور با موفقیت تغییر کرد.'
    oldPassword.value = ''
    newPassword.value = ''
    ...
    passError.value = error.data?.error || 'خطایی رخ داده است. لطفا دوباره تلاش کنید.'
  } finally {
