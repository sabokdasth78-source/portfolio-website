<template>
  <div class="space-y-12 pb-20 pt-10 text-start">
    ...
    <div v-else-if="error || !project" class="text-center py-20 text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/10 rounded-2xl border border-red-200 dark:border-red-500/20">
      {{ t.notFound }}
      <br />
      <NuxtLink to="/projects" class="text-cyan-500 underline mt-4 inline-block">{{ t.backToProjects }}</NuxtLink>
    </div>
          ...
          <div class="lg:col-span-4 space-y-8 relative">
            <div class="sticky top-24">
              <div class="prose prose-slate dark:prose-invert max-w-none bg-slate-50 dark:bg-slate-800/50 p-6 rounded-2xl border border-slate-100 dark:border-slate-700">
                <h3 class="text-2xl font-bold text-slate-800 dark:text-white mb-4">{{ t.aboutProject }}</h3>
                <p class="text-slate-600 dark:text-slate-300 leading-relaxed text-lg whitespace-pre-line text-justify lg:text-start">
                  {{ project.description }}
                </p>
              </div>
...
const t = computed(() => {
  return lang.value === 'en' ? {
    notFound: 'Project not found or an error occurred.',
    ...
    ctaDesc: 'If you liked this project and have an idea for your business, I would be happy to help you implement it.',
    ctaButton: 'Contact & Order Project'
  } : {
    notFound: 'پروژه یافت نشد یا خطایی رخ داده است.',
    backToProjects: 'بازگشت به پروژه‌ها',
    backToList: 'بازگشت به لیست پروژه‌ها',
    videoNotSupported: 'مرورگر شما از پخش ویدیو پشتیبانی نمی‌کند.',
    aboutProject: 'درباره پروژه',
    liveSite: 'مشاهده آنلاین سایت',
    sourceCode: 'سورس کد',
    ctaTitle: 'پروژه‌ای مشابه نیاز دارید؟',
    ctaDesc: 'اگر از این پروژه خوشتان آمده و ایده‌ای برای کسب‌وکار خود دارید، خوشحال می‌شوم در اجرای آن به شما کمک کنم.',
