  ...
  <div class="space-y-24 pb-20">
    ...
    <section class="relative min-h-[85vh] flex items-center bg-white/40 dark:bg-slate-900/40 backdrop-blur-sm rounded-[3rem] p-6 md:p-12 border border-slate-200/50 dark:border-white/5 animate-fade-up overflow-hidden shadow-lg">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center w-full">
        <!-- Text Content -->
        <div class="text-right z-10 space-y-6 md:space-y-8 mt-10 lg:mt-0">
          <div class="inline-block px-4 py-1.5 rounded-full bg-cyan-100 dark:bg-cyan-900/30 text-cyan-700 dark:text-cyan-300 text-xs md:text-sm font-semibold border border-cyan-200 dark:border-cyan-800/50 shadow-sm animate-pulse">
            🚀 آماده برای پروژه‌های جدید و چالش‌های هیجان‌انگیز
          </div>
          <h1 class="text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-extrabold tracking-tight leading-tight text-slate-900 dark:text-white">
            خلق تجربه‌های <br />
            <span class="text-transparent bg-clip-text bg-gradient-to-l from-cyan-500 to-purple-600 dark:from-cyan-400 dark:to-purple-500">خارق‌العاده دیجیتال</span>
          </h1>
          <p class="text-base md:text-lg lg:text-xl text-slate-600 dark:text-slate-400 max-w-xl leading-relaxed">
            من ایده‌های پیچیده را به رابط‌های کاربری زیبا، سریع و کاربرپسند با استفاده از قدرت <span class="font-bold text-emerald-500">Django</span> و ظرافت <span class="font-bold text-green-500">Nuxt.js</span> تبدیل می‌کنم.
          </p>
    ...
    <section class="py-12 md:py-16 relative bg-slate-50/80 dark:bg-slate-950/80 backdrop-blur-md rounded-3xl border border-slate-200 dark:border-white/5 animate-fade-up delay-200 shadow-sm mx-4 md:mx-0">
      ...
      <h2 class="text-2xl sm:text-3xl md:text-4xl font-extrabold mb-8 md:mb-12 text-center text-slate-900 dark:text-white"><span class="text-cyan-500">مهارت‌های</span> من</h2>
    ...
    <section class="animate-fade-up delay-300 p-6 md:p-8 rounded-3xl bg-white/30 dark:bg-slate-800/30 backdrop-blur-xl border border-slate-200/50 dark:border-white/5 mx-4 md:mx-0">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-8 md:mb-12">
        <div>
          <h2 class="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900 dark:text-white mb-2">پروژه‌های <span class="text-purple-500">منتخب</span></h2>
          <p class="text-sm md:text-base text-slate-500 dark:text-slate-400">برخی از کارهایی که اخیراً انجام داده‌ام و به آن‌ها افتخار می‌کنم.</p>
        </div>
        <NuxtLink to="/projects/projects" class="flex text-cyan-600 dark:text-cyan-400 hover:text-cyan-700 dark:hover:text-cyan-300 text-sm font-medium items-center gap-1 group">
          مشاهده همه 
          <svg class="w-4 h-4 rtl:rotate-180 transform group-hover:-translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
        </NuxtLink>
      </div>
      ...
      <div v-else-if="errorProjects" class="text-center py-10 md:py-20 text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/10 rounded-2xl border border-red-200 dark:border-red-500/20">
        خطا در ارتباط با سرور. لطفاً بک‌اِند را بررسی کنید.
      </div>
    ...
    <section class="animate-fade-up delay-400 p-6 md:p-8 rounded-3xl bg-white/30 dark:bg-slate-800/30 backdrop-blur-xl border border-slate-200/50 dark:border-white/5 mx-4 md:mx-0">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-8 md:mb-12">
        <div>
          <h2 class="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900 dark:text-white mb-2">جدیدترین‌های <span class="text-cyan-500">وبلاگ</span></h2>
          <p class="text-sm md:text-base text-slate-500 dark:text-slate-400">مقالاتی در زمینه توسعه وب، طراحی رابط کاربری و تجربیات من.</p>
        </div>
    ...
    <section class="animate-fade-up delay-500 bg-gradient-to-br from-slate-900 to-slate-800 dark:from-slate-800 dark:to-slate-900 rounded-[2rem] md:rounded-[2.5rem] p-8 md:p-16 text-center text-white shadow-2xl relative overflow-hidden border border-slate-700 dark:border-white/10 group mx-4 md:mx-0">
      ...
      <div class="relative z-10">
        <h2 class="text-2xl sm:text-3xl md:text-5xl font-extrabold mb-4 md:mb-6 tracking-tight">پروژه‌ای در ذهن دارید؟</h2>
        <p class="text-sm md:text-lg text-slate-300 mb-8 md:mb-10 max-w-2xl mx-auto leading-relaxed">
          بیایید با هم همکاری کنیم و ایده‌های شما را به واقعیت تبدیل کنیم. من برای فرصت‌های شغلی جدید و پروژه‌های چالش‌برانگیز در دسترس هستم.
        </p>
...
const imageSlides = [
  ...
  { src: '/sliders/slider3.png', link: '/projects/projects' },          
  { src: '/sliders/slider2.png', link: '/contact' }, 
  { src: '/sliders/slider1.png', link: '/projects/projects' }           
];

const config = useRuntimeConfig()

const skills = [
  { name: 'Vue.js / Nuxt 3', level: 'پیشرفته', color: 'bg-green-500' },
  { name: 'Django / Python', level: 'پیشرفته', color: 'bg-emerald-600' },
  { name: 'React.js', level: 'حرفه‌ای', color: 'bg-sky-400' },
  { name: 'Next.js', level: 'حرفه‌ای', color: 'bg-slate-800 dark:bg-slate-400' },
  { name: 'Tailwind CSS v4', level: 'حرفه‌ای', color: 'bg-cyan-400' },
  { name: 'WordPress', level: 'پیشرفته', color: 'bg-blue-600' },
  { name: 'Graphic Design', level: 'حرفه‌ای', color: 'bg-fuchsia-500' },
  { name: 'REST API', level: 'پیشرفته', color: 'bg-purple-500' },
  { name: 'PostgreSQL', level: 'متوسط', color: 'bg-blue-500' },
  { name: 'Docker', level: 'آشنایی', color: 'bg-blue-400' },
  { name: 'Git & GitHub', level: 'پیشرفته', color: 'bg-orange-500' },
  { name: 'UI/UX Design', level: 'متوسط', color: 'bg-pink-500' },
]
